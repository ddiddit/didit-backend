...
enum class RetrospectErrorCode(
    ...
    ATTACHMENT_UNSUPPORTED_FILE(HttpStatus.BAD_REQUEST, "지원하지 않는 파일 형식입니다."),
    ATTACHMENT_SIZE_EXCEEDED(HttpStatus.BAD_REQUEST, "파일 크기가 너무 큽니다. 10MB 이하 파일을 첨부해주세요."),
    ATTACHMENT_INVALID_FILE(HttpStatus.BAD_REQUEST, "파일 정보를 확인할 수 없습니다. 다시 첨부해주세요."),
    ATTACHMENT_ALREADY_BOUND(HttpStatus.CONFLICT, "이미 전송된 첨부파일입니다."),
    ATTACHMENT_NOT_UPLOADED(HttpStatus.CONFLICT, "파일 업로드가 완료되지 않았습니다."),
    ATTACHMENT_LIMIT_EXCEEDED(HttpStatus.BAD_REQUEST, "파일은 한 번에 최대 3개까지 첨부할 수 있습니다."),
    ATTACHMENT_ANALYSIS_FAILED(HttpStatus.SERVICE_UNAVAILABLE, "파일 분석에 실패했습니다. 다시 시도해주세요."),
