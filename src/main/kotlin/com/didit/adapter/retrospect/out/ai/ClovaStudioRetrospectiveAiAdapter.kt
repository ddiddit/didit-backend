...
class ClovaStudioRetrospectiveAiAdapter(
    ...
    private fun callChat(
        ...
        val request = ClovaStudioChatRequest(
        ...
        val response = clovaStudioRestClient.post()
            .uri("/testapp/v1/chat-completions/${clovaStudioProperties.model}")
    ...
    private fun estimateInputTokens(messages: List<ChatMessage>): Int {
        return messages.sumOf { it.content.length / 2 + 1 }
    }

    private fun estimateOutputTokens(content: String): Int {
